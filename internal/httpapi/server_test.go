...
func newTestHandler(t *testing.T, readiness Readiness, assets fstest.MapFS) http.Handler {
	t.Helper()
	handler, err := New(readiness, sharedTestIdentity(t), testRuntimeDependencies(), assets, SecurityConfig{PublicOrigin: testPublicOrigin})
	if err != nil {
...
func TestNewRejectsInvalidSecurityConfiguration(t *testing.T) {
	...
	for _, origin := range []string{"", "http://acmemux.example", "https://AcmeMux.example", "https://acmemux.example/", "https://acmemux.example:443"} {
		if _, err := New(readinessStub{}, sharedTestIdentity(t), testRuntimeDependencies(), fstest.MapFS{}, SecurityConfig{PublicOrigin: origin}); err == nil {
